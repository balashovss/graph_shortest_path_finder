...
void next_iteration_map(cell** map, int height, int width) {
    ...
    free_matrix((void***)(&next_iteration_map_),height, CELL_TYPE);
}
...
void generate_maze(void*** map, int height, int width) {
    maze_cell* tmp = new maze_cell [width];
    *map = create_matrix(height, width, MAZE_CELL_TYPE);
    srand(time(NULL));
    int set_count = 0;
    string_init(&set_count, width, tmp);
...
void all_maze_cells_to_one_set(maze_cell* tmp, int width, int j) {
    ...
    for (int k = j+1; k < width; k++) if ((tmp[k]).set == temp) (tmp[k]).set = (tmp[j]).set;
} 
